...
import ug.co.absa.microsrvc.billsvc.IntegrationTest;
...
class AbsaUgdtMicroBillsSvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/absa-ugdt-micro-bills-svc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-micro-bills-svc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-micro-bills-svc-kafka/unregister"));
                return;
            }
        }
